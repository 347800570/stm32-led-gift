...
IMAGE = ROOT / 'MDK-ARM/MPLT/MPLT.axf'
...
class Firmware:
    ...
    def show(self):
        self.frames.append((self.uc.reg_read(UC_ARM_REG_R1), bytes(
            self.uc.mem_read(self.uc.reg_read(UC_ARM_REG_R0), 67*3))))
        return 1
...
def test_effects():
    ...
    for mode in range(1, 6):
        ...
        for _ in range(150):
            ...
            frames.append(bytes(f.uc.mem_read(address, 201)))
        assert len(set(frames)) > 10, mode
        if mode in (1, 2):
            assert all(frame == frame[:3]*67 for frame in frames)
    f.call('Effects_Reset')
    f.call('Effects_Render', 1, 2, 0, address)
    assert bytes(f.uc.mem_read(address, 201)) == bytes([255, 0, 0])*67
...
def test_led():
    f = Firmware()
    address = 0x20003000
    f.uc.mem_write(address, bytes([255, 128, 64])*67)
    f.call('LED_Init')
    f.us = 2000
    assert f.call('LED_Show', address, 100) == 1
    data = f.transfers[-1]
    assert len(data) == 1866 and data[:2] == b'\0'*2 and data[-256:] == b'\0'*256
