...
def render_temperature(f):
    ...
    output = bytes(f.uc.mem_read(0x20003000, 201))
    assert output == output[:3]*67
    return output[:3]
...
def test_layout_fire():
    ...
    assert set(expected) == set(range(1, 68))
    positions = bytes(f.uc.mem_read(f.symbols['led_positions'], 134))
    actual = [tuple(positions[i:i+2]) for i in range(0, 134, 2)]
    assert actual == [expected[i] for i in range(1, 68)]
    f.call('Effects_Reset')
    output = 0x20003000
    f.call('Effects_Render', 6, 2, 40, output)
    frame = bytes(f.uc.mem_read(output, 201))
    lit = [i for i in range(67) if any(frame[i*3:i*3+3])]
    ...
    for _ in range(100):
        f.call('Effects_Render', 6, 2, 40, output)
        frames.append(bytes(f.uc.mem_read(output, 201)))
    assert len(set(frames)) > 80
    assert any(any(frames[-1][i*3:i*3+3]) for i in range(67) if actual[i][1] == 7)
    print('PASS: all 67 physical coordinates, fire starts low, rises in height, and animates')
...
def test_v02_keys():
    ...
    for code in [0x47, 0x16]:
        f.key(code)
        f.call('App_Task')
        assert f.logs.endswith('UNASSIGNED IN V03\r\n')
