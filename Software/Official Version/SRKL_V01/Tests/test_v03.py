...
def test_pulse_release():
    ...
    assert not any(f.uc.mem_read(0x20003000,222))
...
def test_sound_effects():
    ...
    target = 0x20003000
    coords = bytes(f.uc.mem_read(f.symbols['led_positions'], 148))
    heights = coords[1::2]
    for mode in [8, 9]:
        f.call('Effects_Reset')
        f.call('Effects_Render', mode, 2, 20, target)
        assert not any(f.uc.mem_read(target, 222)), 'silent output must be black'
    level = 128
    for _ in range(5):
        f.call('Effects_Render', 8, 2, 20, target)
    pixels = bytes(f.uc.mem_read(target, 222))
    ...
    a = bytes(f.uc.mem_read(target, 222))
    f.call('Effects_Render', 9, 2, 4000, target)
    b = bytes(f.uc.mem_read(target, 222))
    assert a == a[:3]*74 and b == b[:3]*74 and a != b
    ...
    assert not any(f.uc.mem_read(target, 222))
...
def test_volume_hysteresis():
    ...
    f.stub('Audio_GetLevel', lambda: level)
    coords = bytes(f.uc.mem_read(f.symbols['led_positions'], 148))
    row = [i for i, y in enumerate(coords[1::2]) if y == 6]
    ...
    def render(value, frames=1):
        ...
        for _ in range(frames):
            f.call('Effects_Render', 8, 2, 20, target)
        data = bytes(f.uc.mem_read(target, 222))
    ...
    # Row y=6 turns on at LEVEL=128 and off at LEVEL<=122 (25% band).
    for value in [127,128,126,129,123,125,128]*10:
        visible = render(value)
        assert visible >= 64
        assert visible*3//100 > 0, 'dominant channel stays visible even at 3% global brightness'
    assert render(122, 5) == 0
    for value in [123,125,127]*5:
        assert render(value) == 0, 'inside band an extinguished row stays off'
    assert render(128, 5) > 0
    assert render(0) == 0 and not any(f.uc.mem_read(target,222))
